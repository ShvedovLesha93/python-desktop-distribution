...
if getattr(sys, "frozen", False):
    ...
    # Running as a plain script (dev/testing)
...
def download_release(version: str) -> None:
    ...
    url = f"https://github.com/ShvedovLesha93/python-desktop-distribution/releases/download/{version}/{filename}"
...
def _extract_and_replace(archive_path: Path) -> None:
    ...
    old_exe = APP_DIR / "MyApp"
    backup_exe = APP_DIR / "MyApp.old"

    # Переименовываем запущенный бинарник — Linux позволяет это даже для running process
    if old_exe.exists():
    ...
    try:
        ...
        # Восстанавливаем права на исполняемый файл
        if old_exe.exists():
            old_exe.chmod(0o755)
...
def restart_app() -> None:
    """Перезапускает приложение через новый бинарник."""
    log.info("Restarting app...")
    os.execv(APP_DIR / "MyApp", [APP_DIR / "MyApp"])


def run() -> None:
    download_release("v0.1.0")
